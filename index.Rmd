...
death_data <- read.csv("/home/miqdad/homeless_data/Death.csv", header=TRUE, sep=",", as.is=TRUE, stringsAsFactors=FALSE)
...
renderPlot({
      if(input$yaxis == "Alcohol"){
        q <- ggplot(death_data, aes(x=age_at_death, y=alcohol), fill=practice) +
           geom_point(shape=1, size=3, na.rm=TRUE, aes(color=practice, shape=practice))
      }
      else if(input$yaxis == "Last_Systolic"){
  
        q <- ggplot(death_data, aes(x=age_at_death, y=last_systolic)) +
           geom_point(shape=1, size=3)
      }
      else if(input$yaxis == "Last_Diastolic"){
        q <- ggplot(death_data, aes(x=age_at_death, y=last_diastolic)) +
           geom_point(shape=1, size=3)
      }
      else if(input$yaxis == "Time_Since_Seen"){
        q <- ggplot(death_data, aes(x=age_at_death, y=time_since_seen)) +
           geom_point(shape=1, size=3)
      }
...
population_data <- read.csv("/home/miqdad/homeless_data/Population.csv", header=TRUE, sep=",", as.is=TRUE, stringsAsFactors=FALSE)
...
renderPlot({
          ...
          if(input$cond == "Harmful Alcohol Abuse"){
            ...
            if(input$sex == "Male") {yax="Male_DV"}
            else if(input$sex == "Female") {yax="Female_DV"}
            else if(input$sex == "All") {yax="Total_DV"}  
          }
