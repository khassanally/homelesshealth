...
Death <- read.csv("/home/miqdad/homeless_data/Death.csv", header=TRUE, sep=",", as.is=TRUE, stringsAsFactors=FALSE)
...
renderPlot({
  ##barplot(xtabs(~Death$Classification_of_Death), las=2)
      ...
      {
        Death <- Death %>%
            filter(age_at_aeath>40)
        
        Death <- Death %>%
            filter(age_at_aeath<50)
      }
      else if(input$age_at_death == "40-50")
      {
        Death <- Death %>%
...
renderPlot({
      if(input$yaxis == "Alcohol"){
        q <- ggplot(Death, aes(x=age_at_death, y=alcohol), fill=practice) +
      ...
      q <- q + xlab("Age at Death") + ylab(gsub("_"," ",input$yaxis))
      print(q)
})
...
Population <- read.csv("/home/miqdad/homeless_data/Population.csv", header=TRUE, sep=",", as.is=TRUE, stringsAsFactors=FALSE)
...
renderPlot({
          ...
          t <- ggplot(Population, aes_string("place", tolower(yax))) +
